...
from backend.models import Bill, Ledger, LedgerGroup, Party, Purchase, Voucher, VoucherEntry
...
def post_sales_voucher(session, bill: Bill) -> Voucher:
    ...
    is_credit_sale = bool(getattr(bill, "is_credit", False)) or str(getattr(bill, "payment_mode", "") or "").lower() == "credit"
    if is_credit_sale:
        debtor_ledger = resolve_bill_party_ledger(session, bill) or ledgers["SALES_RECEIVABLE_CONTROL"]
        lines.append({"ledger_id": int(debtor_ledger.id), "entry_type": "DR", "amount": total, "narration": "Customer receivable"})
    posting_cash = round2(min(cash, total)) if not is_credit_sale else 0.0
    posting_online = round2(min(online, max(0.0, total - posting_cash))) if not is_credit_sale else 0.0
    if posting_cash > 0:
    ...
    outstanding = round2(max(0.0, total - posting_cash - posting_online)) if not is_credit_sale else 0.0
    if outstanding > 0:
        debtor_ledger = resolve_bill_party_ledger(session, bill) or ledgers["SALES_RECEIVABLE_CONTROL"]
