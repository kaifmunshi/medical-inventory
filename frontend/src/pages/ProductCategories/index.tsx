...
export default function ProductCategoriesPage() {
  ...
  const updateM = useMutation({
    ...
    onError: (err: any) => toast.push(String(err?.response?.data?.detail || err?.message || 'Failed to update category'), 'error'),
  })
    ...
    <Stack gap={2}>
      ...
      <Paper sx={{ p: 2 }}>
        <Stack direction={{ xs: 'column', md: 'row' }} gap={2} alignItems={{ md: 'center' }}>
          <TextField label="Search categories" value={q} onChange={(e) => setQ(e.target.value)} fullWidth />
      ...
      <Dialog open={open} onClose={() => setOpen(false)} fullWidth maxWidth="xs">
        ...
        <DialogContent dividers>
          <TextField label="Category Name" value={name} onChange={(e) => setName(e.target.value)} fullWidth autoFocus sx={{ mt: 1 }} />
        </DialogContent>
        <DialogActions>
