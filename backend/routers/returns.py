...
# 🔹 summary endpoint: sold, already_returned, remaining (for UI)
...
def create_return(payload: ReturnCreate):
    ...
    with get_session() as session:
        ...
        for line in payload.items:
            ...
            if sold_lookup:
                ...
                if line.quantity > remaining:
                    raise HTTPException(status_code=400, detail=f"Return qty exceeds remaining for {itm.name} (remaining {remaining})")
        ...
        r = Return(
            ...
            refund_cash=rc, refund_online=ro,
            notes=payload.notes
        )
        session.add(r)
        session.commit()
        ...
        # ----- save Return items + restock -----
        try:
            for line in payload.items:
                itm = db_items[line.item_id]
                itm.stock += int(line.quantity)
                session.add(itm)

                line_total = float(itm.mrp) * int(line.quantity)
                if bill:
...
def create_exchange(payload: ExchangeCreate):
    ...
    Returns: { return: ReturnOut, bill: BillOut, net_due }
    """
    ...
    with get_session() as session:
        # ----- bill + proration + remaining logic (same idea as Returns) -----
        sold_lookup = {}
        returned_lookup = {}
        bill = None
        disc_pct = 0.0
        ...
        if payload.source_bill_id:
            ...
            try:
                # prefer stored subtotal; fallback to sum of bill items
                if hasattr(bill, "subtotal") and bill.subtotal is not None:
                    subtotal_bill = float(bill.subtotal)
                else:
                    subtotal_bill = 0.0
        ...
        # 2) Compute new bill totals and stock check (MRP-based; no bill-level discount for new items here)
        new_items_map = {}
        bill_subtotal = 0.0
        ...
        theoretical_net = round2(bill_total - return_subtotal)  # +ve => customer pays; -ve => refund

        # NEW: manual rounding/override from frontend
        rounding_adjustment = round2(getattr(payload, "rounding_adjustment", 0.0) or 0.0)

        # Final net due after adjustment – this is what money actually moves
        ...
        if net_due > 0:
            # customer must PAY net_due via payment_mode
            ...
            # shop must REFUND -net_due (use refund_* fields)
        ...
        notes=payload.notes,
        rounding_adjustment=rounding_adjustment,  # NEW
        )
        ...
        return {
    # what actually moves as cash/refund AFTER rounding
    "net_due": net_due,

    # extra info for UI / debugging
    "theoretical_net": theoretical_net,             # pure math, no manual rounding
    "rounding_adjustment": rounding_adjustment,     # difference applied by user (+/-)
            ...
            "return": ReturnOut(
                ...
                items=[{
                    "item_id": i.item_id, "item_name": i.item_name, "mrp": i.mrp,
