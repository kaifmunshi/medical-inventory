...
from backend.accounting import mark_voucher_deleted, post_purchase_payment_voucher, post_purchase_voucher, sync_purchase_vouchers
...
def assert_purchase_item_untouched(session, purchase_item: PurchaseItem) -> tuple[InventoryLot, Item]:
    if not purchase_item.lot_id:
        raise HTTPException(status_code=400, detail=f"Purchase item #{purchase_item.id} is missing lot linkage")
    lot = session.get(InventoryLot, purchase_item.lot_id)
    if not lot:
        raise HTTPException(status_code=400, detail=f"Lot #{purchase_item.lot_id} not found")
    legacy_item = session.get(Item, lot.legacy_item_id) if lot.legacy_item_id else None
    if not legacy_item:
        raise HTTPException(status_code=400, detail=f"Legacy item missing for lot #{lot.id}")

    total_qty = purchase_item_total_qty(purchase_item)
    if int(lot.sealed_qty or 0) != total_qty or int(lot.loose_qty or 0) != 0:
        raise HTTPException(
            status_code=400,
            detail=f"Purchase item #{purchase_item.id} cannot be edited because stock has already been used or opened",
        )
    if int(legacy_item.stock or 0) != total_qty:
        raise HTTPException(
            status_code=400,
            detail=f"Purchase item #{purchase_item.id} cannot be edited because legacy stock has changed",
        )
    has_open_event = session.exec(
        select(PackOpenEvent.id).where(
            (PackOpenEvent.source_lot_id == lot.id) | (PackOpenEvent.loose_lot_id == lot.id)
        ).limit(1)
    ).first()
    if has_open_event:
        raise HTTPException(
            status_code=400,
            detail=f"Purchase item #{purchase_item.id} cannot be edited because loose stock was created from it",
        )
    return lot, legacy_item
    ...
    with get_session() as session:
            ...
            prepared_items.append(
                {
                    ...
                    "conversion_qty": raw.conversion_qty or product.default_conversion_qty,
                }
            )
        ...
        for entry in prepared_items:
            ...
            legacy_item = Item(
                ...
                mrp=entry["mrp"],
                stock=entry["sealed_qty"] + entry["free_qty"],
            ...
            lot = InventoryLot(
                ...
                rack_number=entry["rack_number"],
                sealed_qty=entry["sealed_qty"] + entry["free_qty"],
                loose_qty=0,
                conversion_qty=entry["conversion_qty"],
        ...
        for payment in payload.payments:
            ...
            post_purchase_payment_voucher(
                ...
                supplier,
                int(payment_row.id),
                float(payment_row.amount or 0),
                bool(payment_row.is_writeoff),
                payment_row.note,
                payment_row.paid_at,
            )
...
def replace_purchase_items(purchase_id: int, payload: PurchaseItemsReplace) -> PurchaseOut:
    ...
    with get_session() as session:
        ...
        touched: List[tuple[PurchaseItem, InventoryLot, Item]] = []
        for item in existing_items:
            lot, legacy_item = assert_purchase_item_untouched(session, item)
            touched.append((item, lot, legacy_item))
        ...
        for purchase_item, lot, legacy_item in touched:
            purchase_item_qty = purchase_item_total_qty(purchase_item)
            lot.sealed_qty = 0
            lot.loose_qty = 0
            lot.is_active = False
            lot.updated_at = now_ts()
            legacy_item.stock = 0
            legacy_item.is_archived = True
            legacy_item.updated_at = now_ts()
            session.add(lot)
            session.add(legacy_item)
...
def cancel_purchase(purchase_id: int) -> PurchaseOut:
    ...
    with get_session() as session:
        ...
        purchase_payments = session.exec(select(PurchasePayment).where(PurchasePayment.purchase_id == purchase.id)).all()
        for payment in purchase_payments:
            source_type = "PURCHASE_WRITEOFF" if bool(payment.is_writeoff) else "PURCHASE_PAYMENT"
            mark_voucher_deleted(session, source_type=source_type, source_id=int(payment.id))
        log_audit(
            session,
            entity_type="PURCHASE",
            entity_id=int(purchase.id),
