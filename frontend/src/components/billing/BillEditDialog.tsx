...
export default function BillEditDialog({
  ...
  useEffect(() => {
    ...
    const lines: EditLine[] = (bill.items || []).map((it: any) => {
      ...
      const custom = qty > 0 ? round2(Number(it.line_total || 0) / qty) : mrp
  ...
  return (
    <>
          ...
          <Stack gap={2}>
            <Paper sx={{ p: 2 }}>
              <Stack direction={{ xs: 'column', md: 'row' }} gap={1.5} alignItems={{ md: 'flex-start' }}>
                    ...
                    <TextField
                      ...
                      helperText={
                        editSelectedCustomer
                          ? [editSelectedCustomer.phone, editSelectedCustomer.address_line].filter(Boolean).join(' | ')
                ...
                />
                <Button variant="outlined" onClick={() => setEditAddCustomerOpen(true)} sx={{ minWidth: { xs: '100%', md: 148 }, height: 40 }}>
                  Add Customer
                </Button>
              </Stack>
            </Paper>
            ...
            <Paper sx={{ p: 2 }}>
              <Stack gap={2}>
                <Stack direction={{ xs: 'column', sm: 'row' }} gap={2} flexWrap="wrap" useFlexGap>
                  <TextField label="Bill Date" type="date" value={editDateTime} onChange={(e) => setEditDateTime(e.target.value)} sx={{ width: 230 }} InputLabelProps={{ shrink: true }} />
                  <TextField
                    select
                    label="Payment Mode"
                ...
                <Stack direction={{ xs: 'column', sm: 'row' }} gap={2} flexWrap="wrap" useFlexGap>
                  {(editPaymentMode === 'cash' || (editPaymentMode === 'split' && editSplitCombination !== 'online-credit')) ? (
                    <TextField label="Cash Amount" type="text" value={String(editCash)} onChange={(e) => handleEditCashAmountChange(e.target.value)} onWheel={blurOnWheel} sx={{ width: 170, ...noSpinnerSx }} inputProps={{ inputMode: 'decimal', pattern: '[0-9]*[.,]?[0-9]*' }} />
                  ) : null}

                  {(editPaymentMode === 'online' || (editPaymentMode === 'split' && editSplitCombination !== 'cash-credit')) ? (
                    <TextField label="Online Amount" type="text" value={String(editOnline)} onChange={(e) => handleEditOnlineAmountChange(e.target.value)} onWheel={blurOnWheel} sx={{ width: 170, ...noSpinnerSx }} inputProps={{ inputMode: 'decimal', pattern: '[0-9]*[.,]?[0-9]*' }} />
                  ) : null}
      ...
      <Dialog open={editAddCustomerOpen} onClose={() => setEditAddCustomerOpen(false)} maxWidth="sm" fullWidth>
        <DialogTitle>Add Customer</DialogTitle>
        <DialogContent dividers>
          <Stack gap={2} mt={1}>
            <TextField label="Name" value={editNewCustomerName} onChange={(e) => setEditNewCustomerName(e.target.value)} required fullWidth />
            <TextField
              ...
              fullWidth
            />
            <TextField label="Address (optional)" value={editNewCustomerAddress} onChange={(e) => setEditNewCustomerAddress(e.target.value)} fullWidth />
          </Stack>
        </DialogContent>
        <DialogActions>
